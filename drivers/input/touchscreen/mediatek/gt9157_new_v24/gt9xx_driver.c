...
#define ATTR_FOR_AMOI_ENGINEER_MODE 		1
//*********************add by huangwb @20140910***********//

//#define GESTURE_ENABLE		
static int gesture_enable=0;  //0: disable ; 1 enable
...
static struct proc_dir_entry *gt91xx_gesture_proc = NULL;	//add by huangwb @20140910 

//static int set_tp_protect(bool flag);
...
static s32 gtp_get_sensor_id(struct i2c_client *client)
{
    s32 ret = -1;
    g_sensor_id[0] = GTP_REG_SENSOR_ID >> 8;
    g_sensor_id[1] = GTP_REG_SENSOR_ID & 0xff;
    ret = gtp_i2c_read(client, g_sensor_id, 3);
//	ret = gtp_i2c_read_dbl_check(client, GTP_REG_SENSOR_ID, &g_sensor_id[GTP_ADDR_LENGTH], 1);
    if (ret < 0) {
        GTP_ERROR("Read SENSOR ID failed,default use group1 config!");
        g_sensor_id[GTP_ADDR_LENGTH] = 0;
        goto out;
    }
    g_sensor_id[GTP_ADDR_LENGTH] &= 0x03;
//	GTP_DEBUG("huangwb g_sensor_id= %d \n",g_sensor_id[GTP_ADDR_LENGTH]);
out:
    return ret;
}

static ssize_t show_tp_sensor_id(struct device_driver *driver, char *buf)
{
    return sprintf(buf, "sensor_id-->[%d]\n", g_sensor_id[GTP_ADDR_LENGTH]);
}

static DRIVER_ATTR(sensor_id, S_IWUSR | S_IRUGO, show_tp_sensor_id, NULL);
static struct driver_attribute *gtp_attr_list[] = {
    &driver_attr_sensor_id,
};

static int gtp_create_attr(struct device_driver *driver)
{
    int idx, err = 0;
    int num = (int)(sizeof(gtp_attr_list)/sizeof(gtp_attr_list[0]));
    if (driver == NULL) {
        return -EINVAL;
    }

    for(idx = 0; idx < num; idx++) {
        if((err = driver_create_file(driver, gtp_attr_list[idx]))) {
            GTP_DEBUG("driver_create_file (%s) = %d\n", gtp_attr_list[idx]->attr.name, err);
            break;
        }
    }
    return err;
...
static int gt91xx_gesture_write_proc(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
	char temp[2] = {0};
	
//	GTP_DEBUG("huangwb write count %d\n", count);
...
//static int gt91xx_gesture_read_proc(struct file *file, char *buffer, size_t count, loff_t *ppos)
static ssize_t gt91xx_gesture_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{/*
	char temp[2] = {0};
	int len=0;
	if(gesture_enable==1)
...
static const struct file_operations gt_gesture_proc_fops = { 
    .write = gt91xx_gesture_write_proc,
    .read = gt91xx_gesture_read_proc
};
...
//***********************add by huangwb @20140910***************************************//
	gt91xx_gesture_proc = proc_create(GT91XX_GESTURE_PROC_FILE, 0660, NULL, &gt_gesture_proc_fops);
    if (gt91xx_config_proc == NULL)
    {
        GTP_ERROR("create_proc_entry %s failed\n", GT91XX_GESTURE_PROC_FILE);
    }
//******************************end*****************************************************//
   ...
   u8  rqst_data[3] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
    ...
    do
    {
        ...
        if (DOZE_ENABLED == doze_status)
        {
            ...
            {     
                if (
				/*	(doze_buf[2] == 'a') || (doze_buf[2] == 'b') ||(doze_buf[2] == 'z')  ||
                   		 (doze_buf[2] == 'd') || (doze_buf[2] == 'e') || (doze_buf[2] == 'g') || 
                  	  	(doze_buf[2] == 'h') || (doze_buf[2] == 'm') || (doze_buf[2] == 'q') ||
                    		(doze_buf[2] == 's') || (doze_buf[2] == 'y') ||(doze_buf[2] == 0x5E) || /* ^ */
                    (doze_buf[2] == 'w') ||(doze_buf[2] == 'v')  || (doze_buf[2] == 'c') ||(doze_buf[2] == 'o')
