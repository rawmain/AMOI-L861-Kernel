...
//amoi lincg@20141229 for accdet swap begin 
#define AMOI_HEADSET_SWITCH
#define PIN_HEADSET_SW (GPIO143 | 0x80000000)
static int swapstate = 0;
//amoi lincg@20141229 end
/*----------------------------------------------------------------------
            ...
            if(current_status == 0)
            {
				...
				pmic_pwrap_write(ACCDET_PWM_WIDTH, cust_headset_settings->pwm_width);
				 ...
				 if (current_status == 0 && show_icon_delay != 0)
				 {
				  ...
				  mutex_lock(&accdet_eint_irq_sync_mutex);
				  if(1 == eint_accdet_sync_flag) {
			     //amoi lincg@20141219 Modify headset Switch @20140303
				#if defined(AMOI_HEADSET_SWITCH)
				if(swapstate == 0)
				{
					ACCDET_DEBUG("[Accdet]The headset switch Start!!!swapstate = %d\n",swapstate);
					mt_set_gpio_mode(PIN_HEADSET_SW, GPIO_MODE_GPIO);
					mt_set_gpio_dir(PIN_HEADSET_SW,GPIO_DIR_OUT);
					mt_set_gpio_out(PIN_HEADSET_SW,GPIO_OUT_ONE);
					swapstate = 1;
					ACCDET_DEBUG("[Accdet]The headset switch OK!!!swapstate = %d\n",swapstate);
				}
				else
				{
					ACCDET_DEBUG("[Accdet]The headset switch Start!!!swapstate = %d\n",swapstate);
					mt_set_gpio_mode(PIN_HEADSET_SW, GPIO_MODE_GPIO);
					mt_set_gpio_dir(PIN_HEADSET_SW,GPIO_DIR_OUT);
					mt_set_gpio_out(PIN_HEADSET_SW,GPIO_OUT_ZERO);
					swapstate = 0;
					ACCDET_DEBUG("[Accdet]The headset switch OK!!!swapstate = %d\n",swapstate);
				}	
				#endif				
...
//amoi lincg@20150618  create proc entry at  /proc/headset_type begin
#if defined(AMOI_HEADSET_SWITCH) 
...
static int mt_headset_type_show(struct seq_file *m, void *v)
{
    char headsetType[10]="";
	if(cable_type == HEADSET_MIC) {
		if(swapstate == 0) {
            strcpy(headsetType,"4CN \n");
		} else {
            strcpy(headsetType,"4US \n");
		}
	} else if(cable_type == NO_DEVICE) {
        strcpy(headsetType,"0No \n");
	} else {
        strcpy(headsetType,"3XX \n");       
	}
    
	seq_printf(m, "%s", headsetType);
	return 0;
}

static int mt_headset_type_open_proc(struct inode *inode, struct file *file)
{
	return single_open(file, mt_headset_type_show, NULL);
}

static const struct file_operations headset_type_ops = {
	.open  = mt_headset_type_open_proc, 
	.read  = seq_read,    
};
#endif
//amoi lincg@20150618 create proc entry at  /proc/headset_type  end
...
//#endif
    ...
    entry = proc_create("headset_type", 0444, NULL, &headset_type_ops);
    if (entry == NULL)
    {
        printk("create_proc_entry headset_type failed\n");
    }
    else
    {
        printk("create proc entry headset_type success");
    }
