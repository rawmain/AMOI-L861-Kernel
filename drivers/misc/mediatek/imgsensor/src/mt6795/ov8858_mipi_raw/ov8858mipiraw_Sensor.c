...
static void sensor_init(void)
{
	...
	write_cmos_sensor( 0x3018, 0x32); //2lane
	write_cmos_sensor( 0x3020, 0x93);
	write_cmos_sensor( 0x3022, 0x01);
	write_cmos_sensor( 0x3031, 0x0a);
	write_cmos_sensor( 0x3034, 0x00);
...
// ++++++++  @@5.1.1.2 Raw 10bit 1632x1224 30fps 2lane 672M bps/lane
	// ++++++++  ;;
	// ++++++++  ;; MIPI=672Mbps, SysClk=72Mhz,Dac Clock=360Mhz.
	// ++++++++  ;;
	// ++++++++  ;Pclk 72MHz
...
write_cmos_sensor( 0x380f, 0xdc);  // ; VTS L
...
static void capture_setting(kal_uint16 currefps)
{
	...
	// +++++++	@@5.1.1.3 Raw 10bit 3264*2448 15fps 2lane 672M bps/lane Dac Clock=180Mhz
	// +++++++	;Pclk 72MHz ,Tline = 8.981481us 
	...
	// +++++++	;lines per frame=2474(0x9aa)
		...
		write_cmos_sensor( 0x380f, 0xaa);  // ; VTS L
		write_cmos_sensor( 0x3813, 0x02);  // ; view not same modify
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	set_mirror_flip(imgsensor.mirror);//wujian add
	mdelay(10);
	return ERROR_NONE;
}	/*	preview   */
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
			get_default_framerate_by_scenario((MSDK_SCENARIO_ID_ENUM)*feature_data, (*(feature_data+1)));
			break;
