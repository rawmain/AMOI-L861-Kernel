...
#define PROC_CAMERA_INFO "main_camera_name"
...
/*********************add by shenzw for camera debug @20150619 begin************************/
static char g_procSensorName[32] = KDIMGSENSOR_NOSENSOR;
#define SENSOR_NAME_MAX_NUM                 10
typedef struct
{
   UINT8  SensorName[32];
   UINT8  SensorNum;
...
} SENSOR_NAME_STRUCT, *PSENSOR_NAME_STRUCT;

SENSOR_NAME_STRUCT SensorNameList[SENSOR_NAME_MAX_NUM] =
{
   //{SENSOR_DRVNAME_OV8825_MIPI_RAW,0x01,0x01,"TR",0x01},//8825 
   //{SENSOR_DRVNAME_OV8820_MIPI_RAW,0x02,0x01,"TR",0x01},//8820
   //{SENSOR_DRVNAME_OV5647MIPI_RAW,0x03,0x02,"TR",0x01},//5647   
   //{SENSOR_DRVNAME_OV12830_MIPI_RAW,0x04,0x05,"SY",0x01},//sy 12830   
   {SENSOR_DRVNAME_IMX214_MIPI_RAW,0x05,0x05,"TR",0x01},//xl imx214 
   {SENSOR_DRVNAME_OV12830_MIPI_RAW,0x04,0x04,"TR",0x01},//xl 12830   
   {SENSOR_DRVNAME_OV13850_MIPI_RAW,0x06,0x07,"SY",0x01},//sy 13850   
   //{SENSOR_DRVNAME_OV8865_MIPI_RAW,0x07,0x09,"TR",0x01},//xl 8865   
   //{SENSOR_DRVNAME_OV13851_MIPI_RAW,0x06,0x08,"TR",0x01},//xl 13850   
   {SENSOR_DRVNAME_OV8865_MIPI_RAW,0x07,0x09,"TR",0x01},//xl 8865 fno 2.2
   //{SENSOR_DRVNAME_OV8867_MIPI_RAW,0x07,0x09,"TR",0x02},//xl (8867)8865 fno 2.0
   {SENSOR_DRVNAME_OV8858_MIPI_RAW,0x05,0x08,"TR",0x01},//xl (8867)8858 fno 2.0
   {SENSOR_DRVNAME_IMX230_MIPI_RAW,0x09,0x11,"TR",0x01},//imx230
};



static int amoi_camera_info_show(struct seq_file *m, void *v)
{
	char strCamerainfoShow[50]={0};
	char *p=NULL;
	UINT8 num=0xFF; 
	int  i=0;
	for(i=0;i<SENSOR_NAME_MAX_NUM;i++)
	{
	  if(strcmp(g_procSensorName,SensorNameList[i].SensorName)==0)
	  	{
	  	  num=i;
		  break;
	  	}
	...
	if(num!=0xFF)
		seq_printf(m,"CAMERA_%02x_%02x_%s_%02x\n",SensorNameList[num].SensorNum,SensorNameList[num].CameraNum,SensorNameList[num].Factory,SensorNameList[num].Batch);
	else
...
static int amoi_camera_info_open_proc(struct inode *inode, struct file *file)
{
	return single_open(file, amoi_camera_info_show, NULL);
}

static const struct file_operations amoi_camera_info_ops = { 
	.open  = amoi_camera_info_open_proc, 
	.read  = seq_read,        
};
/*****************shenzw end @20150619****************************/
/*******************************************************************************
...
inline static int adopt_CAMERA_HW_CheckIsAlive(void)
{
	...
	for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
		...
		if (sensorID == 0) {    /* not implement this feature ID */
			...
			//add by shenzw @20150619
			if(DUAL_CAMERA_MAIN_SENSOR==g_invokeSocketIdx[i]) 
			{
			    memcpy((char*)g_procSensorName,(char*)g_invokeSensorNameStr[i],32);
				
				struct proc_dir_entry *entry = NULL;
				entry = proc_create(PROC_CAMERA_INFO, S_IRUGO, NULL, &amoi_camera_info_ops);
				if (entry == NULL)
				{
					printk("proc_create camera_info failed\n");
				}
				else
				{
					printk("proc_create camera_info success");
				}
			}
			//end by shenzw @20150619
