...
static stAF_DrvList g_stAF_DrvList[MAX_NUM_OF_LENS] = {
	...
	{1, AFDRV_DW9810AF, DW9810AF_SetI2Cclient, DW9810AF_Ioctl, DW9810AF_Release},
	#endif
...
static int AF_Open(struct inode *a_pstInode, struct file *a_pstFile)
{
    ...
    if (!g_pstAF_CurDrv)  // interface to 3.10.72 camera_isp
    	LOG_INF("Set Motor Name\n");
		g_pstAF_CurDrv = &g_stAF_DrvList[0];
		g_pstAF_CurDrv->pAF_SetI2Cclient(g_pstAF_I2Cclient, &g_AF_SpinLock, &g_s4AF_Opened);
...
static int __init AF_i2C_init(void)
{
	#if I2C_CONFIG_SETTING == 1
...
static void __exit AF_i2C_exit(void)
{
	platform_driver_unregister(&g_stAF_Driver);
}
module_init(AF_i2C_init);
module_exit(AF_i2C_exit);
