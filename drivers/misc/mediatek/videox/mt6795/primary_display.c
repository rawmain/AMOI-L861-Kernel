...
int _build_path_memout_mira(void)
{
	...
	pgc->dpmgr_sub_mem_path_handle = dpmgr_create_path(DDP_SCENARIO_SUB_GAMMA_MEMOUT, pgc->cmdq_handle_sub_mem_config);
    ...
    if(ret == 0)
    {
        DISPCHECK("config M4U Port %s to %s SUCCESS\n",ddp_get_module_name(DISP_MODULE_OVL1), primary_display_use_m4u?"virtual":"physical");
    }
    else
    {
    ...
    if(ret)
    {
        DISPCHECK("config M4U Port %s to %s FAIL(ret=%d)\n",ddp_get_module_name(DISP_MODULE_WDMA1), primary_display_use_m4u?"virtual":"physical", ret);
        return -1;
    }
#endif // MTKFB_NO_M4U
...
static int primary_display_esd_check_worker_kthread(void *data)
{
	...
	while(1)
	{
		...
		#else
		if(count==0)//for dsi don't recive te workarround
		{   
		    count++;
			msleep(4000);
		}
...
static unsigned int keeplow=0;
static int od_by_pass = 0;
...
int _display_set_lcm_refresh_rate(int fps)
{
   ...
   primary_display_idlemgr_kick(__func__, 0);
	...
	if(fps == 120)
	{
		...
		{
		    DISPCHECK("[refresh_rate]step1: adjust high vcores\n");
		    mmdvfs_set_step(SMI_BWC_SCEN_120HZ, MMDVFS_VOLTAGE_HIGH);
		}
	}
	else
	{
    ...
    if(cmdq_handle == NULL){
        ...
        if(ret!=0)
        {
         ...
         DISPCHECK("[refresh_rate]step2: primary_display_set_lcm_refresh_rate:fps(%d)\n", fps);
		 ...
		 if(fps == 120)
		 {
		     // do before clk change
		 }
		 else
		 {
			 DISPCHECK("[refresh_rate]step3: adjust low vcores\n");
		     mmdvfs_set_step(SMI_BWC_SCEN_120HZ, MMDVFS_VOLTAGE_LOW);
		 }
...
static int trigger_decouple_mirror()
{
    if(pgc->need_trigger_dcMirror_out == 0){
        DISPMSG("There is no output config when decouple mirror!!\n");
    } else {
