...
{
	PK_DBG("amoi hwpoweron: pwInfo.PowerType = %d, pwInfo.voltage = %d \n ",pwInfo.PowerType,pwInfo.Voltage);
    ...
    else if(pwInfo.PowerType==SensorMCLK)
    {
        ...
        {
			PK_DBG("Sensor MCLK3 On");
			ISP_MCLK3_EN(TRUE);
        }
    }
	else if(pwInfo.PowerType==AFEN)
    {
        PK_DBG("hwPowerOn: AFEN %d \n",pwInfo.Voltage);
        
        if(mt_set_gpio_mode(CAMERA_AF_EN_PIN, CAMERA_AF_EN_PIN_M_GPIO)){PK_DBG("[CAMERA LENS_AFEN] set gpio mode failed!! \n");}
        if(mt_set_gpio_dir(CAMERA_AF_EN_PIN,GPIO_DIR_OUT)){PK_DBG("[CAMERA LENS_AFEN] set gpio dir failed!! \n");}
        if(pwInfo.Voltage == Vol_High)
        {           
            if(mt_set_gpio_out(CAMERA_AF_EN_PIN,GPIO_OUT_ONE)){PK_DBG("[CAMERA LENS AFEN] set gpio failed!! \n");}
        }
        else
        {           
            if(mt_set_gpio_out(CAMERA_AF_EN_PIN,GPIO_OUT_ZERO)){PK_DBG("[CAMERA LENS AFEN] set gpio failed!! \n");}
        }
    }
    else{}
    if(pwInfo.Delay>0)
...
int kdCISModulePowerOn(CAMERA_DUAL_CAMERA_SENSOR_ENUM SensorIdx, char *currSensorName, BOOL On, char* mode_name)
{
	...
	printk("kdCISModulePowerOn: enter!!, on = %d,name = %s \n", On,currSensorName);
