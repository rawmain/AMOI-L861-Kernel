...
static unsigned int I2CDMAWriteBuf_pa = 0;	/* = NULL; */
static char *I2CDMAReadBuf = NULL;
static unsigned int I2CDMAReadBuf_pa = 0;	/* = NULL; */
...
static int mt6605_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    usleep_range( 900, 1000); //debug
//	gpio_mode = mt_get_gpio_mode(GPIO_NFC_RST_PIN);
//	printk("mt6605_probe gpio_mode = %d\n",gpio_mode);
	/* EINT_gpio */
	...
	printk("%s :I2CDMAWriteBuf_pa %d, I2CDMAReadBuf_pa,%d\n", __func__, I2CDMAWriteBuf_pa,
		 I2CDMAReadBuf_pa);
...
void mt6605_dev_irq_handler(void)
{
	...
	printk("%s : &mt6605_dev=%p\n", __func__, mt6605_dev);

	if (NULL == mt6605_dev) {
		printk("mt6605_dev NULL\n");
		return;
	}
	mt6605_disable_irq(mt6605_dev);
	...
	printk("%s : wake_up &read_wq=%p\n", __func__, &mt6605_dev->read_wq);
	printk("mt6605_dev_irq_handler\n");
}
...
static ssize_t mt6605_dev_read(struct file *filp, char __user *buf, size_t count, loff_t *offset)
{
	...
	//printk("%s : %d, &mt6605_dev=%p", __func__, count, mt6605_dev);
	...
	if (!mt_get_gpio_in(mt6605_dev->irq_gpio)) {
		...
		if (forceExitBlockingRead == 1) {
			printk("%s :forceExitBlockingRead.\n", __func__);
		...
		printk("%s : wait_event_interruptible ret %d,gpio,%d\n", __func__, ret,
			 mt_get_gpio_in(mt6605_dev->irq_gpio));
		...
		if (ret || forceExitBlockingRead) {
			...
			mutex_lock(&mt6605_dev->read_mutex);
			if (forceExitBlockingRead == 1) {
				printk("%s : clear flag for forceExitBlockingRead\n", __func__);
				forceExitBlockingRead = 0;	/* clear flag */
			}
			mutex_unlock(&mt6605_dev->read_mutex);
			goto fail;
				...
				loff_t *offset)
{
	...
	do {
		...
		printk("%s : writing %zu bytes, remain bytes %d.\n", __func__, count,
			 count_remain);
		...
		if (ret == count_ori) {
			...
			//printk("%s :remain_bytes, %d,%d,%d,%d,%d\n", __func__, ret_tmp, ret,
				 //count, count_ori, idx);
		}
	...
	printk("%s : writing %d bytes. Status %d\n", __func__, count_ori, ret);
	return ret;
}
...
//	gpio_dir = mt_get_gpio_dir(GPIO_NFC_RST_PIN);
//	gpio_level = mt_get_gpio_in(GPIO_NFC_RST_PIN);
//	gpio_mode = mt_get_gpio_mode(GPIO_NFC_RST_PIN);

//	printk("mt6605_dev_open gpio_dir = %d,gpio_level = %d,gpio_mode = %d\n",gpio_dir,gpio_level,gpio_mode);

	printk("mt6605_dev_open,%s : %d,%d, &mt6605_dev_open=%p\n", __func__, imajor(inode),
...
static long mt6605_dev_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	printk("mt6605_dev_unlocked_ioctl,cmd,0x%x,0x%lx\n", cmd, arg);
	/* (((arg & 0xFF00) >> 8) & 0x00FF), */
	/* (arg & 0x00FF)); */
	...
	if ((cmd & 0xFFFF) == 0xFE00) {
		mt_eint_mask(CUST_EINT_IRQ_NFC_NUM);
		printk("mt6605_dev_unlocked_ioctl,mask_IRQ");
		return 0;
		...
		mt_eint_mask(CUST_EINT_IRQ_NFC_NUM);
		printk("mt6605_dev_unlocked_ioctl,Re-registered IRQ");
		return 0;
	} else if ((cmd & 0xFFFF) == 0xFEFF)	/* EXIT EINT */
	{
		...
		mutex_unlock(&mt6605_dev->read_mutex);
		wake_up_interruptible(&mt6605_dev->read_wq);
		printk("mt6605_dev_unlocked_ioctl,SW Release IRQ");
		return 0;
	...
	if (tmp_gpio == MTK_NFC_GPIO_EN_B) {
        ...
        switch (command) {
            case MTK_NFC_IOCTL_CMD_CLOCK_BUF_ENABLE :
                //only for MT6735
                printk("mt6605_dev_unlocked_ioctl, enable clock buffer. ");
                //do nothing
                break;
            case MTK_NFC_IOCTL_CMD_CLOCK_BUF_DISABLE :
                //only for MT6735
                printk("mt6605_dev_unlocked_ioctl, disable clock buffer. "); 
                //do nothing.
                break;
            case MTK_NFC_IOCTL_CMD_EXIT_EINT:
                printk("mt6605_dev_unlocked_ioctl, EXIT EINT.");
        		mutex_lock(&mt6605_dev->read_mutex);
        		forceExitBlockingRead = 1;
        		mutex_unlock(&mt6605_dev->read_mutex);
        		wake_up_interruptible(&mt6605_dev->read_wq);
            ...
            case MTK_NFC_IOCTL_CMD_READ_DATA:
                printk("Call mt6605_dev_irq_handler. irq=%d. \n",
                    mt_get_gpio_in(mt6605_dev->irq_gpio));
                if (mt_get_gpio_in(mt6605_dev->irq_gpio)) {                    
	...
	if (result != MTK_NFC_PULL_INVALID) {
		if (cmd == MTK_NFC_IOCTL_READ) { /* READ */
            ...
            if (tmp_gpio == MTK_NFC_GPIO_IRQ && result == 0x01) {
                printk("mt6605_dev_unlocked_ioctl, irq is high call mt6605_dev_irq_handler.\n");
                mt6605_dev_irq_handler();
            }
	...
	printk("mt6605_dev_unlocked_ioctl,%d\n", result);
	return result;
#endif 
//	return 0;
}
...
{ 
    ...
    printk("inform_nfc_vsim_change, md_id,%d, md_state,%d, vsim_state,%d , send_data[6],0x%X.\n",
    ...
    for (retry = 0; retry < 10; retry++)
    {
        ...
        if (ret == send_bytes)
        {
            printk("inform_nfc_vsim_change, send to mt6605 OK. retry %d", retry);
            break;
        }
        else
        {
            printk("inform_nfc_vsim_change, send to mt6605 fail. retry %d, ret %d", retry, ret);
        }    
        //sleep 2ms
        msleep(2);
    }
